    ...
    .map(([key, value]) => {
      ...
      return `
				<div class="variable-item is-collapsed">
					<span class="variable-key" data-variable="${escapeHtml(key)}">${escapeHtml(
        cleanKey
      )}</span>
...
export function makeUrlAbsolute(
  element: Element,
    ...
    try {
      ...
      if (!["http:", "https:"].includes(url.protocol)) {
        ...
        if (firstSegment && firstSegment.includes(".")) {
          ...
          const newUrl =
            `${baseUrl.protocol}//` + attributeValue.split("://")[1];
...
export function processUrls(htmlContent: string, baseUrl: URL): string {
  ...
  tempDiv
    ...
    .forEach((img) => makeUrlAbsolute(img, "srcset", baseUrl));
  tempDiv
    .querySelectorAll("img")
    .forEach((img) => makeUrlAbsolute(img, "src", baseUrl));
  tempDiv
    .querySelectorAll("a")
    .forEach((link) => makeUrlAbsolute(link, "href", baseUrl));
