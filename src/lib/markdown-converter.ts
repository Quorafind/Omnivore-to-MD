// Credit goes to https://github.com/obsidianmd/obsidian-clipper
...
export function createMarkdownContent(content: string, url: string) {
  ...
  turndownService.addRule("table", {
    ...
    replacement: function (content, node) {
      ...
      const hasComplexStructure = Array.from(
        node.querySelectorAll("td, th")
      ).some(
  ...
  turndownService.addRule("listItem", {
    ...
    replacement: function (
      ...
      if (parent instanceof HTMLOListElement) {
        ...
        let index =
          Array.from(parent.children).indexOf(node as HTMLElement) + 1;
    ...
    replacement: function (content, node) {
      ...
      const items = Array.from(node.children).map((item, index) => {
        if (item instanceof HTMLElement) {
          const itemContent = item.innerHTML.replace(
      ...
      if (node instanceof HTMLElement) {
        ...
        if (footnoteId && footnoteContent) {
          ...
          footnotes[footnoteId] = turndownService.turndown(
            decodeURIComponent(
              footnoteContent.replace(/&lt;/g, "<").replace(/&gt;/g, ">")
            )
          );
  ...
  turndownService.addRule("removals", {
    filter: function (node) {
      ...
      if (
        ...
        return (
          node.textContent?.trim() === "#" || node.querySelector("img") !== null
        );
      }
  ...
  turndownService.addRule("preformattedCode", {
    ...
    replacement: (content, node) => {
      ...
      let codeContent = codeElement
        ? extractStructuredText(codeElement)
  ...
  function extractLatex(element: Element): string {
    ...
    const mathNode =
      element.nodeName.toLowerCase() === "math"
        ? element
      ...
      if (
        ...
        (node.getAttribute("display") === "block" ||
          ...
          (node.parentElement &&
            ...
            node.parentElement.previousElementSibling.nodeName.toLowerCase() ===
              "p"))
      ) {
        return `\n$$$\n${latex}\n$$$\n`;
      } else {
